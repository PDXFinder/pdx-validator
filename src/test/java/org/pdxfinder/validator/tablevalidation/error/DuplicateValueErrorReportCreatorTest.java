...
public class DuplicateValueErrorReportCreatorTest {
  ...
  @Test
  public void checkUniqueValues_givenUniqueValues_emptyErrorList() {
    Map<String, Table> tableSetWithUniqueValues = new HashMap<>();
    Table tableWithUniqueValues =
        completeTableSet
            .get(TABLE_1)
            .addColumns(StringColumn.create("unique_col", Arrays.asList("1", "2")));
    ...
    assertThat(
        duplicateValueErrorCreator
            .generateErrors(tableSetWithUniqueValues, tableSetSpecification)
            .isEmpty(),
        is(true));
  }

  @Test
  public void checkUniqueValues_givenDuplicateValues_addsDuplicateValueErrorToErrorList() {
    Table tableWithUniqueValues =
        completeTableSet
            .get(TABLE_1)
            .addColumns(StringColumn.create("unique_col", Arrays.asList("1", "1", "2")));
    ...
    TableSetSpecification tableSetSpecification =
        TableSetSpecification.create().setProvider(PROVIDER).addUniqueColumns(uniqueCol);
    ...
    assertEquals(
        expected.toString(),
        duplicateValueErrorCreator
            .generateErrors(tableSetWithDuplicateValues, tableSetSpecification)
            .toString());
  }
}
